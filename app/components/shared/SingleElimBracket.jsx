...
import { Trophy, ChevronDown, ChevronRight, Zap } from 'lucide-react';
...
export default function SingleElimBracket({ matches, players, onMatchClick }) {
  // Sort matches into QF, SF, GF
  const qfMatches = matches.filter(m => m.round === 'QF').sort((a,b) => a.label.localeCompare(b.label));
  const sfMatches = matches.filter(m => m.round === 'SF').sort((a,b) => a.label.localeCompare(b.label));
  const gfMatches = matches.filter(m => m.round === 'GF').sort((a,b) => a.label.localeCompare(b.label));
  ...
  return (
    <div className="w-full h-full flex flex-col gap-2 p-2 sm:p-4 pb-20">
      ...
      <div className="text-center mb-6 mt-4 relative">
        ...
        <p className="text-muted-foreground text-sm uppercase tracking-widest font-bold relative z-10">Single Elimination Stage</p>
      </div>

      <RoundGroup 
        title="⚔️ Quarterfinals" 
        matches={qfMatches} 
        players={players} 
