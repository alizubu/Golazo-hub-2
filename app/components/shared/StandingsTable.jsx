...
export function computeStandings(matches, players, seasonId, config = {}) {
  ...
  const isPlayoff = m.round !== 'league' && m.round !== 'groupA' && m.round !== 'groupB';
...
export default function StandingsTable({ matches, players, seasonId, me, onPlayerClick, onH2HClick, config = {}, headerLeft, seasonName, seasonType }) {
  const tableRef = useRef(null);
  const [isExporting, setIsExporting] = useState(false);
  
  let groups = [];
  if (seasonType === '8-Player Tournament') {
    const groupAMatches = matches.filter(m => m.round === 'groupA');
    const groupBMatches = matches.filter(m => m.round === 'groupB');
    
    const getGroupPlayers = (groupMatches) => {
      const ids = new Set();
      groupMatches.forEach(m => { ids.add(m.homeId); ids.add(m.awayId); });
    ...
    groups.push({
      ...
      players: getGroupPlayers(groupAMatches),
      isKnockoutLimit: 1, // Top 2 qualify
      qualifyText: "Top 2 Qualify for Knockout Stage"
    });
    
    groups.push({
      name: 'Group B',
      matches: groupBMatches,
      players: getGroupPlayers(groupBMatches),
      isKnockoutLimit: 1, // Top 2 qualify
      qualifyText: "Top 2 Qualify for Knockout Stage"
    });
  } else {
    groups.push({
      name: seasonName || "Premier League",
      matches: matches,
      players: players,
      isKnockoutLimit: 3, // Top 4 qualify
      qualifyText: "Top 4 Qualify for Knockout Stage"
    });
  }
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div ref={tableRef} className="flex flex-col gap-4 bg-gradient-to-b from-[#0a0c10] to-[#12151b] p-2 sm:p-3 rounded-xl border border-border/40 shadow-[0_8px_30px_rgb(0,0,0,0.12)]">
        {groups.map((g, idx) => {
          const standings = computeStandings(g.matches, g.players, seasonId, config);
          const completedCount = g.matches.filter(m => m.seasonId === seasonId && m.status === 'completed').length;
          const totalMatchesCount = g.matches.filter(m => m.seasonId === seasonId).length;
          const matchday = Math.floor(completedCount / (Math.max(1, g.players.length) / 2)) || 1;

          return (
            <div key={idx} className="overflow-x-auto rounded-xl border border-border/20 bg-card/40 pb-4 pt-2 px-0 sm:px-2">
              ...
              <div className="px-4 py-3 flex flex-wrap items-center justify-between gap-4 border-b border-border/30 mb-2">
                ...
                <div className="flex items-center gap-4 text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                  <div className="flex items-center gap-1.5"><Users size={14} className="text-pitch" /> {g.players.length} Players</div>
                ...
                <tbody className="divide-y divide-white/[0.03] text-xs">
                  <AnimatePresence>
                    {standings.map((s, i) => {
                      ...
                      const isKnockoutZone = i === g.isKnockoutLimit && standings.length > (g.isKnockoutLimit + 1);
