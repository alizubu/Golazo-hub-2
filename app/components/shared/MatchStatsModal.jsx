...
function StatBar({ label, valueA, valueB, colorA, colorB, format = 'number', index }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex w-full h-2.5 sm:h-3 bg-secondary bg-zinc-900 rounded-full overflow-hidden shadow-inner ring-1 ring-white/5">
        <motion.div 
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4">
        ...
        <motion.div 
          className="relative w-full max-w-[420px] bg-background bg-[#0f1117] sm:rounded-2xl rounded-t-2xl shadow-2xl overflow-hidden border-t sm:border border-border/50 max-h-[90vh] flex flex-col"
        ...
        >
          ...
          <div className="absolute top-3 right-3 z-10 flex gap-2">
            <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-full bg-card/40 text-foreground hover:bg-secondary/70 bg-black/60 transition-colors">
              <X size={18} />
            </button>
          </div>
