...
function NavItem({ item, active, hasLive, onClick, isMobile = false, isExpanded = true }) {
  ...
  return (
    ...
    >
      {/* Hover Background */}
      <div className={`absolute inset-y-0 rounded-xl bg-black/5 bg-white/[0.03] opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${isMobile ? 'inset-x-3' : (isExpanded ? 'inset-x-3' : 'inset-x-2')}`} />
...
function SidebarFooter({ activeSeason, onNavigate, onLogout, isLoggingOut, isMobile = false, isExpanded = true }) {
  ...
  return (
    <div className={`border-t border-border border-white/[0.04] flex-shrink-0 relative overflow-hidden transition-all duration-300 ${showText ? 'p-4' : 'p-2'}`}>
      <div className="absolute inset-0 bg-gradient-to-t from-black/5 from-white/[0.02] to-transparent pointer-events-none" />
      
      {/* Season chip */}
      {activeSeason && (
        <div className={`items-center gap-3 px-4 py-3 rounded-xl bg-black/5 bg-white/[0.03] border border-border/50 mb-3 transition-colors hover:bg-black/10 bg-white/[0.05] ${showText ? 'flex' : 'hidden'}`}>
      ...
      <button
        ...
        className={`
          ...
          text-muted-foreground hover:text-foreground hover:bg-black/5 bg-white/[0.04] transition-colors outline-none
  ...
  return (
    <>
      ...
      <aside
        ...
        className={`hidden md:flex flex-col fixed left-0 top-0 bottom-0 z-[50] bg-background bg-[#0A0D14] border-r border-border border-white/[0.04] shadow-2xl transition-all duration-300 ${isExpanded ? 'w-[260px]' : 'w-16'}`}
      >
        ...
        {/* Logo */}
        <div className={`flex items-center h-14 border-b border-border border-white/[0.04] flex-shrink-0 relative z-10 transition-all duration-300 ${isExpanded ? 'justify-start px-5 gap-3' : 'justify-center px-0'}`}>
        ...
        {mobileOpen && (
          <>
            <motion.div
              ...
              className="md:hidden fixed inset-0 z-[70] bg-background/80 bg-[#0A0D14]/80 backdrop-blur-md"
              aria-hidden="true"
            />
            ...
            <motion.aside
              ...
              className="md:hidden fixed left-0 top-0 bottom-0 z-[80] w-[280px] bg-background bg-[#0A0D14] border-r border-border border-white/[0.04] shadow-2xl flex flex-col"
            ...
            >
              ...
              <button
                onClick={() => setMobileOpen(false)}
                className="absolute top-4 right-4 z-20 w-8 h-8 flex items-center justify-center rounded-full bg-black/5 bg-white/5 border border-border text-foreground hover:bg-white/10 transition-colors"
                aria-label="Close navigation"
              >
                <X size={14} />
              </button>

              <div className="flex items-center gap-3 h-14 px-5 border-b border-border border-white/[0.04] flex-shrink-0 relative z-10">
                <div className="flex items-center justify-center w-8 h-8 rounded-xl bg-gold/10 border border-gold/20 shadow-[0_0_15px_rgba(217,169,59,0.15)] flex-shrink-0 relative overflow-hidden">
                   <div className="absolute inset-0 bg-gradient-to-br from-white/10 to-transparent" />
                   <Trophy size={16} className="text-[#D9A93B]" />
                </div>
