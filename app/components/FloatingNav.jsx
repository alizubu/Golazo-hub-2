...
export default function FloatingNav({ session, me, players = [], notifications = [], matches = [] }) {
  ...
  // Wait, if it's admin, they don't see FloatingNav? The original code had:
  ...
  return (
    <>
      ...
      <Dialog open={searchOpen} onOpenChange={setSearchOpen}>
        <DialogContent className="p-0 gap-0 w-screen h-screen sm:h-auto max-w-full sm:max-w-lg sm:rounded-xl bg-zinc-950 border-zinc-800 sm:shadow-2xl overflow-hidden m-0 sm:border-solid z-[99999]">
          <DialogHeader className="sr-only">
            <DialogTitle>Search</DialogTitle>
          </DialogHeader>
          <Command className="bg-transparent">
            <CommandInput placeholder="Search players, tabs..." className="h-12 text-sm border-b border-zinc-800 rounded-none text-white" />
                ...
                {items.map(it => {
                  ...
                  return (
                    <CommandItem
                      ...
                      className="flex items-center gap-2 rounded-lg cursor-pointer py-2 text-zinc-300 hover:text-white"
                    >
